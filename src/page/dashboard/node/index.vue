<template>
    <div class="dashboard-node">
        <a-card title="统计">
            <template #title>
                <span>节点</span>
                <a-button type="text" size="large" @click="getNodeState()">
                    <template #icon>
                        <icon-refresh/>
                    </template>
                </a-button>
        ...
        <a-card v-for="item in nodeItems" :title="item.name" style="margin-top: 7px;">
            ...
            <a-descriptions :column="1">
                ...
                <a-descriptions-item label="内存">
                    ...
                    <a-progress
                            :percent="Math.floor(item.node.os.mem.used_in_bytes / item.node.os.mem.total_in_bytes * 100) / 100"
                            :status="nodeStateMemStatus(item.node.os.mem.used_in_bytes, item.node.os.mem.total_in_bytes)"/>
                </a-descriptions-item>
            </a-descriptions>
        </a-card>
    </div>
