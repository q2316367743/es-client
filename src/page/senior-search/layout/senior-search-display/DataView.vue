    ...
    <div class="base-scroll" :class="jsonWrap ? 'json-wrap' : ''">
      ...
      <div class="current-scroll" v-show="itemActive === -1">
        ...
        <div class="json-scroll"
          ...
          <a-scrollbar class="scrollbar"
                       :style="{ fontSize: Optional.ofNullable(jsonFontSize).orElse(16) + 'px' }">
            <base-view v-if="view === ViewTypeEnum.BASE" :value="show"/>
            <json-view v-else-if="view === ViewTypeEnum.JSON" :value="show" :copy="false"/>
            <json-tree-view v-else-if="view === ViewTypeEnum.JSON_TREE" :value="show"/>
          </a-scrollbar>
        </div>
        <monaco-view v-if="view === ViewTypeEnum.EDITOR" :value="show"/>
        <table-viewer v-else-if="view === ViewTypeEnum.TABLE" :data="show"/>
      </div>
    </div>
...
export default defineComponent({
  ...
  computed: {
    ...
    ...mapState(useSeniorSearchStore, ['show'])
  },
  setup() {
  ...
  methods: {
    execCopy() {
      ...
      utools.copyText(jsonFormat(this.show));
    ...
    fixAdd() {
      ...
      this.items.push({
        title: this.index + '',
        view: this.view,
        data: typeof this.show === 'object' ? this.show : {},
      });
    },
